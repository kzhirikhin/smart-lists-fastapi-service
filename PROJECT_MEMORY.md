...
**Последнее обновление:** 2026-09-03 (`INSIGHTS_MODEL` — переключение модели для сравнения)
...
8. Асинхронный Anthropic client вызывает модель, выбранную `INSIGHTS_MODEL`
   (по умолчанию `claude-haiku-4-5-20251001`; см. «Выбор модели»), и пишет её
   идентификатор в лог. Если кешированный access-токен истёк, клиент сначала
   берёт у metadata-сервера ID-токен и обменивает его — примерно раз в десять
   минут, а не на каждый запрос.
...
| `INSIGHTS_MODEL` | необязательна | имя записи в `MODEL_CHOICES`: `haiku` (default) или `sonnet`. Несекретна |

Секретов в списке нет ни одного: конфигурация сервиса состоит из
идентификаторов и адресов. Обязательны все, кроме `DEBUG` и `INSIGHTS_MODEL`:
без остальных сервис не может ни пустить вызывающего, ни обратиться к
Anthropic, и падение на старте честнее, чем ответ ошибкой на каждый запрос.

### Выбор модели

`INSIGHTS_MODEL` задаёт **имя записи** в `MODEL_CHOICES` (`app/services/ai.py`),
а не идентификатор модели. Разница существенна: произвольная строка не может
стать моделью, поэтому опечатка в ревизии не уйдёт в Anthropic именем
несуществующей модели, а чужое имя — счётом по чужому прайсу. Неизвестное
значение не роняет сервис: применяется `haiku`, расхождение видно в логе
предупреждением. Невыставленная переменная воспроизводит прежнее поведение
ровно.

Записи различаются не только идентификатором. У `sonnet` явно передаётся
`thinking={"type": "disabled"}`, потому что Sonnet 5 при пропуске этого
аргумента включает adaptive thinking сам: черновик начал бы делить
`max_tokens=2048` с ответом, а при исчерпании лимита текстового блока в ответе
не осталось бы вовсе и запрос завершился бы ошибкой. У Haiku 4.5 умолчание
обратное, поэтому лишний аргумент не передаётся. Набор допустимых ключей любой
записи ограничен `model` и `thinking` и проверяется обходом самого реестра —
см. A39.

Переменная заведена ради сравнения моделей на настоящих списках и уедет вместе
со сравнением. Измерения на синтетическом наборе из 16 сценариев: Haiku ≈ $3.4
на тысячу инсайтов при 5.8 с, Sonnet 5 ≈ $7.9 при 9.3 с, Sonnet 4.6 ≈ $12.3
при 13.3 с; заметного выигрыша в качестве у старших моделей не обнаружено,
поэтому default остаётся `haiku`. Размышления на этой задаче не окупаются:
adaptive thinking сам отказывается от них на всех трёх моделях, а
принудительный бюджет добавляет расход без изменения ответа.
