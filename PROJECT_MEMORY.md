...
**Последнее обновление:** 2026-09-18 (production Vertex AI и очистка legacy env)
...
С 2026-09-18 production обслуживает Vertex AI; прежняя Anthropic-ревизия и
внешнее правило federation пока сохраняются до завершения этапа 9. Deploy workflow
удаляет четыре legacy `ANTHROPIC_*` переменные из Cloud Run; локальный Compose
их больше не передаёт. Архивирование внешнего правила выполняется отдельно.
...
- 2026-09-04: для миграции на Vertex AI выбрана `gemini-3.5-flash-lite`.
  ...
  1.839/2.938 с и оценку $0.474 на тысячу запросов, 3.5 — 1.777/2.814 с и
  $0.793. На основном наборе 3.5 не дала качественного выигрыша, но добавочный
  peak-набор из четырёх запросов у всех верхних API-границ показал более точный
  разбор статусов по связанным подпунктам: 3.5 дала p50 3.760 с и $8.325/1000,
  3.1 — 4.130 с и $6.683/1000. Поэтому рекомендация сместилась к 3.5, а
  пользователь подтвердил выбор 3.5 при закрытии этапа 4. Методика лежит в
  `benchmarks/vertex-flash-lite-2026-09-04.md`. Production пока остаётся на
  Haiku; выбор применяется только на отдельном этапе замены провайдера.
