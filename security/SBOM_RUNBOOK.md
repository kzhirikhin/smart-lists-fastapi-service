...
Что именно пересобирает образ, задаёт `paths` в `deploy.yml` — allowlist из
входов Dockerfile, самого workflow и его verifier. Всё прочее — тесты,
документация, policy-файлы, коллекции bruno — выкладку не запускает, и это
свойство держит `TestDeployTriggers`, сверяя allowlist с `COPY` из Dockerfile.
До 09-02 фильтр был перечнем исключений и промахивался по любому новому пути:
тестовый PR №57 пересобрал production и снял покрытие 27 exact VEX-statements
с работавшего digest.
