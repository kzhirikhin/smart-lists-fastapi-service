...
Порядок разбора красного run и границы всего контура описаны в
[`security/SBOM_RUNBOOK.md`](../SBOM_RUNBOOK.md).
...
Пустые списки VEX и waiver являются нормальным безопасным состоянием. Текущее
фактическое состояние смотри в digest-файлах этого каталога и
`security/waivers.json`: оно меняется только отдельным review-PR.
