...
#
# --only-binary=:all: — pip-эквивалент npm-ного `--ignore-scripts`. Колёса
# при установке код не исполняют, а sdist исполняет `setup.py`, то есть даёт
# произвольное выполнение от одного факта установки. Флаг запрещает sdist
# целиком: проверено, что все пакеты набора ставятся колёсами.
#
